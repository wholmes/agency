...
export function LogoMark({ size = 32, className = "", style }: { size?: number; className?: string; style?: React.CSSProperties }) {
  /**
   * "The Meeting" mark — two isometric faces sharing a vertical edge.
   *
   * Geometry: a cube corner seen straight-on.
   *   Left face  — slightly darker fill (depth)
   *   Right face — slightly lighter fill (light)
   *   Top ridge  — single bright stroke, the meeting edge
   *
   * At 28px (nav) it reads as a clean diamond-chevron.
   * At 48px+ the two planes are clearly distinct.
   * The concept (brand meets code = two disciplines at one edge) is
   * in the geometry, not a caption.
   *
   * All coordinates in a 32×32 viewBox.
   * Centre of the mark: (16, 16).
   *
   * Isometric cube corner:
   *   Top point:    (16,  5)
   *   Left point:   ( 5, 11.5)
   *   Right point:  (27, 11.5)
   *   Bottom point: (16, 27)
   *   Mid-left:     ( 5, 11.5) → (16, 18)
   *   Mid-right:    (27, 11.5) → (16, 18)
   */
  return (
    <svg
      width={size}
      ...
      aria-hidden="true"
    >
      {/* Left face — warm shadow side */}
      <path
        d="M16 5 L5 11.5 L16 18 L16 5Z"
        fill="currentColor"
        fillOpacity="0.55"
      />
      {/* Right face — light side */}
      <path
        d="M16 5 L27 11.5 L16 18 L16 5Z"
        fill="currentColor"
        fillOpacity="0.90"
      />
      {/* Bottom — base, darkest */}
      <path
        d="M5 11.5 L16 18 L27 11.5 L16 27 L5 20 Z"
        fill="currentColor"
        fillOpacity="0.28"
      />
      {/* Ridge — the meeting edge, full brightness */}
      <line
        x1="16" y1="5"
        x2="16" y2="18"
        stroke="currentColor"
