...
import { getIndustrySlugs, getProjects, getServiceDetailSlugs } from "@/lib/cms/queries";

export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const baseUrl = "https://brandmeetscode.com";
  const lastModified = new Date();
  const [projects, serviceSlugs, industrySlugs] = await Promise.all([
    getProjects(),
  ...
  return [...staticPages, ...servicePages, ...industryPages, ...workEntries];
}
